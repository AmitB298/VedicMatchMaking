<#
.SYNOPSIS
  Production-grade Node.js Docker server initializer.
.DESCRIPTION
  Ensures package.json, package-lock.json, and server.js exist. Generates production-ready server.js if missing.
.PARAMETER Force
  Overwrites existing server.js if present.
.EXAMPLE
  .\Initialize-NodeServer.ps1
#>
...
$LogFile = Join-Path $PWD "node-server-initialize-log.txt"
$Log = @()
...
Log "------------------------------------------------------------"
Log "✅ Node.js Docker Server Initializer"
Log "------------------------------------------------------------"
...
if (!(Test-Path $PackageJson)) {
    ...
    Log "➡️ You need to initialize a Node project first:"
    Log "   npm init -y"
    Save-Log
    exit 1
}
...
# 6️⃣ Auto-generate server.js with correct backtick
