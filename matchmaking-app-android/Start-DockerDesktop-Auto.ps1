...
try {
    ...
    Log "Raw WSL --list --verbose output:"
    Log $wslList

    # Flatten all lines into tokens so it works even if the listing is one giant line
    $allTokens = $wslList -join ' ' -split '\s+'
    $dockerDesktopIndex = $allTokens.IndexOf('docker-desktop')
    $dockerDesktopState = if ($dockerDesktopIndex -gt -1 -and ($dockerDesktopIndex + 1) -lt $allTokens.Count) {
        $allTokens[$dockerDesktopIndex + 1]
    } else {
        ""
    }

    if ($dockerDesktopIndex -eq -1) {
    ...
    if ($dockerDesktopState -match "Stopped") {
        ...
        Log "➡️ ACTION: Will attempt to start Docker Desktop to boot the VM."
        $NeedsStart = $true
    } elseif ($dockerDesktopState -match "Running") {
        Log "✔️ 'docker-desktop' WSL2 VM is already RUNNING."
        $NeedsStart = $false
    } else {
        Log "❓ Unknown state for docker-desktop: '$dockerDesktopState'"
        $NeedsStart = $true
    }
...
if ($NeedsStart) {
    ...
    do {
        ...
        $wslListCurrent = wsl --list --verbose
        $tokensCurrent = $wslListCurrent -join ' ' -split '\s+'
        $indexCurrent = $tokensCurrent.IndexOf('docker-desktop')
        $stateCurrent = if ($indexCurrent -gt -1 -and ($indexCurrent + 1) -lt $tokensCurrent.Count) {
            $tokensCurrent[$indexCurrent + 1]
        } else {
            ""
        }

        Log "Polling docker-desktop WSL2 state: '$stateCurrent' after $Elapsed seconds."

        if ($stateCurrent -match "Running") {
